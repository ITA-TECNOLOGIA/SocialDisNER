...
def results_disease_predictor(
    ...
    if isinstance(model, list):
        model_name = f"best{len(model)}uptodate"
    else:
    ...
    results, results_empty = disease_predictor.read_file_and_save_tsv_results(
        input_path=config.VALIDATION_DATASET_V4_PATH, output_path=output_path
    )
    ...
    assert len(results_grouped_df) == 2500
...
if __name__ == "__main__":
    ...
    if MODEL:
        ...
        results_disease_predictor(
            ...
            include_model_predict=True,
            remove_emojis=False,
        )  # , dictionary=config.VAL_GAZETTEER_ES_FULL_PATH
